...
app.post("/api/resume/edit", isLoggedIn, upload.single("cvPic") , async (req, res) => {
  const {
    firstName,
    lastName,
    about,
  ...
  try {
    ...
    let fullURL;
    if(req.file) {
      const result = await cloudinary.uploader.upload(req.file.path , { folder : "JobNest"})
...
app.post( "/api/employer/profile/avatar", isLoggedIn, upload.single("avatar"), async (req, res) => {
    ...
    try {
      ...
      await foundUser.save()
       res.status(201).json({ message: "resume created successfully", user: foundUser });
    } catch (error) {
      return res.status(500).json({ error: "Error creating user." });
...
app.post( "/api/company/edit" , isLoggedIn , upload.single("logo"),  async (req, res) => {
...
app.post( "/api/job/post" , isLoggedIn ,  async (req, res) => {
    const { title, company, description, education,experience , age , salary , incentiveOne , incentiveTwo , location } = req.body;
    console.log(req.body);
...
app.post( "/api/job/preview" ,  async (req, res) => {
    ...
    try {
      ...
      res
        .status(201)
        .json({ message: "job found successfully", job: job });
    } catch (error) {
      return res.status(500).json({ error: "Error creating user." });
    }
  }
);
app.post( "/api/interview/invite" ,isLoggedIn , upload.none() ,  async (req, res) => {
    ...
    try {
      ...
      res
        .status(201)
        .json({ message: "interview sent successfully" });
    } catch (error) {
      return res.status(500).json({ error: "Error creating user." });
    }
  }
...
app.post( "/api/job/reject" ,isLoggedIn ,  async (req, res) => {
    ...
    try {
      ...
      const interview = await interviewModel.findOne({job: job._id , applicant : applicantID});
      ...
      res
        .status(201)
        .json({ message: "rejected successfully" });
    } catch (error) {
      return res.status(500).json({ error: "Error creating user." });
    }
  }
...
app.post( "/api/resume/preview" ,  async (req, res) => {
    ...
    try {
      ...
      res
        .status(201)
        .json({ message: "job found successfully", resume: resume });
    } catch (error) {
      return res.status(500).json({ error: "Error creating user." });
    }
  }
